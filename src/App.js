...
class App extends Component {
  ...
  constructor(){
    super();
  this.client= Stitch.initializeDefaultAppClient('myshop-nareq');
  this.client.callFunction('Greet',['Yudha']);
  ...
  authHandler = (event, authData) => {
    ...
    if (this.state.authMode === 'login') {
      const credentials=new UserPasswordCredential(authData.email,authData.password)
       request = this.client.auth.loginWithCredential(credentials)
    } else {
  ...
  request.then(result=>{
    ...
    this.errorHandler("An error occured.");
          console.log(err);
          this.setState({ isAuth: false });
  })
